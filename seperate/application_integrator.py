...
class EnhancedUI:
    ...
    def integrate_enhanced_features(self):
        ...
        # Add word count validation - skip layout modification that causes conflicts
        self.setup_word_count_validation_safe()
    ...
    def setup_word_count_validation_safe(self):
        """Set up word count validation for text fields - safe version that avoids layout conflicts"""
        # Create word limit variables without adding new labels
        ...
        # Update existing word count variables instead of creating new ones
        def enhanced_raw_word_count(event=None):
            self.validate_word_limit(self.app.raw_statement, 500, self.app.raw_word_count)
            
        def enhanced_generated_word_count(event=None):
            self.validate_word_limit(self.app.generated_statement, 500, self.app.generated_word_count)
        
        # Override update word count functions
        self.app.update_raw_word_count = enhanced_raw_word_count
        self.app.update_generated_word_count = enhanced_generated_word_count
    ...
    def enhance_progress_indicator(self):
        ...
        def enhanced_refresh():
            ...
            try:
                ...
                log_error("Enhanced refresh error", e)
    ...
    def add_dropdown_suggestions(self, entry_widget, suggestions):
        ...
        try:
            ...
            # For safety, use place method instead of grid or pack
            if not hasattr(entry_widget, 'dropdown_button'):
                # Create dropdown button without messing with layout
                def add_dropdown_button():
                    try:
                        # Get position info for the entry
                        x = entry_widget.winfo_rootx() + entry_widget.winfo_width() - 20
                        y = entry_widget.winfo_rooty()
                        
                        # Create a dropdown button using place instead of grid
                        button = ttk.Button(self.app.root, text="▼", width=2, 
                                          command=lambda: show_suggestions())
                        button.place(x=x, y=y, width=20, height=entry_widget.winfo_height())
                ...
                # We'll add the button after the window is fully configured
                self.app.root.after(500, add_dropdown_button)
        except Exception as e:
