...
class MPStatementRewriter:
    def __init__(self, root):
        ...
        # Load sample data if needed - run AFTER database initialization
    ...
    def initialize_openai(self):
        ...
        try:
            ...
            if not api_key:
                ...
                if not os.path.exists('config.ini'):
                    ...
                    messagebox.showwarning(
                        "API Key Required", 
                        "Please add your OpenAI API key in a .env file or edit the config.ini file."
                    )
            ...
            messagebox.showerror("Missing Dependency", 
                               "The openai or dotenv package is not installed. Please run: pip install openai python-dotenv")
    ...
    def populate_sample_data(self):
        ...
        try:
            ...
            # Check if past_responses table is empty
    ...
    def save_api_settings(self, api_key, model, window):
        """Save the API settings to config file and environment"""
        try:
            ...
            if api_key and not api_key.startswith('•'):
                # Instead of saving in config, create or update .env file
                ...
                # Save a placeholder in config
                config['API']['OPENAI_API_KEY'] = 'stored_in_env_file'
            ...
            messagebox.showinfo("Settings Saved", "API settings updated successfully.")
            self.status_var.set("API settings updated.")
        except Exception as e:
    ...
    def call_llm_api(self, prompt):
        ...
        try:
            response = self.openai.ChatCompletion.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert political communications specialist who rewrites official government statements into personalized MP communications that sound authentic, engaging, and locally relevant."},
    ...
    def call_refresh_llm_api(self, prompt):
        ...
        try:
            response = self.openai.ChatCompletion.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert political communications specialist who rewrites statements to sound authentic, engaging, and locally relevant."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7,
                max_tokens=1500
...
def main():
    ...
    try:
        ...
        # Apply enhanced UI features - MOVED AFTER APP IS FULLY INITIALIZED
        try:
            # Delay the EnhancedUI initialization to ensure UI components exist
            root.after(100, lambda: initialize_enhanced_ui(app, root))
        except Exception as e:
            log_error("Enhanced UI setup error", e)
...
def initialize_enhanced_ui(app, root):
    """Initialize enhanced UI features after main UI is created"""
    try:
